...
class TrinityPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `trinity`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM trinity WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO trinity (date, title, link, content)"
